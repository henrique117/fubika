...
async def get_leaderboard_scores(
    ...
    # Define o bitwise do ScoreV2
    SCORE_V2_BIT = 536870912 

    # --- 1. A QUERY PRINCIPAL DO LEADERBOARD ---
    query = [
        ...
        # MUDANÇA 1: Aceita Status 2 (Best Global) E Status 3 (Best V2)
        "AND s.status IN (2, 3) " 
        
        # MUDANÇA 2: Filtra APENAS scores que tenham o mod V2 ativado
        "AND (s.mods & :v2_mod) != 0 "
    ...
    params: dict[str, Any] = {
        ...
        "user_id": player.id,
        "mode": mode,
        "v2_mod": SCORE_V2_BIT, # Passamos o valor do mod
    }
    ...
    # MUDANÇA 3 (Opcional): ScoreV2 geralmente ordena por SCORE, não PP.
    # Se quiser forçar, troque _score por score na linha abaixo.
    ...
    # --- 2. O SCORE PESSOAL (SEU SCORE NO RODAPÉ) ---
    if score_rows:  # None or []
        ...
        personal_best_score_row = await app.state.services.database.fetch_one(
            ...
            # MUDANÇA 4: Busca seu melhor score V2 (Status 2 ou 3)
            "AND status IN (2, 3) "
            "AND (mods & :v2_mod) != 0 "
        ...
        if personal_best_score_row is not None:
            # --- 3. CÁLCULO DO RANKING (QUE POSIÇÃO EU TO?) ---
            p_best_rank = 1 + await app.state.services.database.fetch_val(
                "SELECT COUNT(*) FROM scores s "
                "INNER JOIN users u ON u.id = s.userid "
                "WHERE s.map_md5 = :map_md5 AND s.mode = :mode "
                
                # MUDANÇA 5: Conta quantas pessoas com V2 estão na sua frente
                "AND s.status IN (2, 3) "
                "AND (s.mods & :v2_mod) != 0 "
                "AND u.priv & 1 "
...
async def getScores(
    ...
    if not requesting_from_editor_song_select:
        ...
        # Filtra para aparecer apenas scores com V2 na LB

        if bmap.status == 2 or bmap.status == 5:
            score_rows = [
                s for s in score_rows 
                if (s['mods'] & Mods.SCOREV2)
            ]

            if personal_best_score_row:
                if not (personal_best_score_row['mods'] & Mods.SCOREV2):
                    personal_best_score_row = None
    else:
        score_rows = []
        personal_best_score_row = None
