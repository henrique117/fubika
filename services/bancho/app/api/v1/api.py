...
async def get_player_v1_robust(username: str | None = None, user_id: int | None = None) -> dict | None:
    """
    Versão da V1 para busca robusta de jogadores.
    Tenta o nome original, se falhar e tiver espaço, tenta com '+'.
    """
    if user_id:
        return await users_repo.fetch_one(id=user_id)
    
    if not username:
        return None

    decoded_name = unquote(username).strip()
    
    user = await users_repo.fetch_one(name=decoded_name)
    if user:
        return user
    
    if " " in decoded_name:
        alt_name = decoded_name.replace(" ", "+")
        user = await users_repo.fetch_one(name=alt_name)
        if user:
            return user
...
async def api_get_player_info(
    ...
    user_info = await get_player_v1_robust(username=username, user_id=user_id)
...
async def api_get_player_status(
    ...
    if not player:
        ...
        row = await get_player_v1_robust(username=username, user_id=user_id)
...
async def api_get_player_scores(
    ...
    """Retorna uma lista dos scores recentes ou melhores de um determinado utilizador."""
    ...
    if mode_arg in (
        ...
        return ORJSONResponse(
            {"status": "Modo de jogo inválido."},
            status_code=status.HTTP_400_BAD_REQUEST,
        )

    if username and user_id:
        return ORJSONResponse(
            {"status": "Deves fornecer ou o ID ou o Nome, não ambos!"},
            status_code=status.HTTP_400_BAD_REQUEST,
        )

    if username:
        player_dict = await get_player_v1_robust(username=username)
        if player_dict:
            player = await app.state.sessions.players.from_cache_or_sql(id=player_dict["id"])
        else:
            player = None
    elif user_id:
        ...
        return ORJSONResponse(
            {"status": "Deves fornecer id ou name!"},
            status_code=status.HTTP_400_BAD_REQUEST,
        )

    if not player:
        return ORJSONResponse(
            {"status": "Jogador não encontrado."},
            status_code=status.HTTP_404_NOT_FOUND,
        )
...
async def api_get_player_most_played(
    ...
    if user_id is not None:
        ...
        # Apply robust name handling: try original name, then with '+' if it has spaces
        decoded_name = unquote(username).strip()
        player = await app.state.sessions.players.from_cache_or_sql(name=decoded_name)
        
        if not player and " " in decoded_name:
            alt_name = decoded_name.replace(" ", "+")
            player = await app.state.sessions.players.from_cache_or_sql(name=alt_name)
    else:
        return ORJSONResponse(
            {"status": "Must provide either id or name."},
            status_code=status.HTTP_400_BAD_REQUEST,
