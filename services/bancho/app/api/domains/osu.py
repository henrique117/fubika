...
def authenticate_player_session(
    ...
    async def wrapper(
        ...
        core_name = decoded_name.strip(" _+")
        ...
        if " " in decoded_name:
            names_to_try.append(decoded_name.replace(" ", "+"))
            names_to_try.append(decoded_name.replace(" ", "_"))
        ...
        names_to_try = [name for name in set(names_to_try) if name]
        ...
        if "@" in decoded_name:
            from app.repositories import users as users_repo
            user = await users_repo.fetch_one(email=decoded_name, fetch_all_fields=True)
            if user:
                user_pw_hash = user["pw_bcrypt"].encode()
                if app.state.cache.bcrypt[user_pw_hash] == pw_md5.encode():
                    player = await app.state.sessions.players.get_sql(id=user["id"])
                    if player:
                        return player
...
async def osuSearchHandler(
    ...
    # Bypass Auth with robust name handling
    decoded_name = unquote(username)
    
    # Extract core name (without leading/trailing space/_/+)
    core_name = decoded_name.strip(" _+")
    
    # Generate all possible name variations
    names_to_try = [
        decoded_name,              # Original input
        core_name,                 # Without separators
        " " + core_name,           # With leading space
        "_" + core_name,           # With leading underscore
        "+" + core_name,           # With leading plus
    ]
    
    # Add replacements of separators in the original
    if " " in decoded_name:
        names_to_try.append(decoded_name.replace(" ", "+"))
        names_to_try.append(decoded_name.replace(" ", "_"))
    if "_" in decoded_name:
...
async def osuSubmitModularSelector(
    ...
    try:
        ...
        if unique_id1_md5 != player.client_details.uninstall_md5:
            raise ValueError(f"unique_id1 mismatch ({unique_id1_md5} != {player.client_details.uninstall_md5})")

        if unique_id2_md5 != player.client_details.disk_signature_md5:
            raise ValueError(f"unique_id2 mismatch ({unique_id2_md5} != {player.client_details.disk_signature_md5})")
        ...
        if score.client_checksum != server_score_checksum:
            raise ValueError(f"online score checksum mismatch ({server_score_checksum} != {score.client_checksum})")

        if bmap_md5 != updated_beatmap_hash:
            raise ValueError(f"beatmap hash mismatch ({bmap_md5} != {updated_beatmap_hash})")
        ...
        if score.status == SubmissionStatus.BEST:
            ...
            if score.bmap.has_leaderboard:
                ...
                score.player.enqueue(
                    app.packets.notification(
                        f"Você alcançou #{score.rank}! ({performance})",
                    ),
                )
                ...
                if score.rank == 1 and not score.player.restricted:
                    ...
                    ann = [
                        f"\x01ACTION alcançou o #1 em {score.bmap.embed}",
                        f"com {score.acc:.2f}% para {performance}.",
                    ]
                    ...
                    if prev_n1:
                        if score.player.id != prev_n1["id"]:
                            ann.append(
                                f"(Anterior #1: https://{app.settings.DOMAIN}/u/"
    ...
    if score.passed:
        ...
        if len(replay_data) >= 24:
            ...
            log(f"{score.player} submeteu score sem replay!", Ansi.LRED)
            if not score.player.restricted:
                await score.player.restrict(admin=app.state.sessions.bot, reason="score sem replay")
    ...
    if score.passed and score.bmap.has_leaderboard:
        ...
        if score.bmap.awards_ranked_pp and score.status == SubmissionStatus.BEST:
            ...
            if score.prev_best:
                ...
                if score.grade != score.prev_best.grade:
                    if score.grade >= Grade.A:
                        ...
                        stats_updates[format(score.grade, "stats_column")] = stats.grades[score.grade]
                    if score.prev_best.grade >= Grade.A:
                        stats.grades[score.prev_best.grade] -= 1
                        stats_updates[format(score.prev_best.grade, "stats_column")] = stats.grades[score.prev_best.grade]
            else:
                if score.grade >= Grade.A:
                    stats.grades[score.grade] += 1
                    stats_updates[format(score.grade, "stats_column")] = stats.grades[score.grade]
            ...
            bonus_acc = 100.0 / (20 * (1 - 0.95 ** len(best_scores))) if best_scores else 1.0
    ...
    if not score.player.restricted:
        ...
        await app.state.services.database.execute(
            ...
            {"plays": score.bmap.plays, "passes": score.bmap.passes, "map_md5": score.bmap.md5},
        )
    ...
    if not score.passed:
        ...
        if score.bmap.awards_ranked_pp and not score.player.restricted:
            ...
            player_achievements = await user_achievements_usecases.fetch_many(user_id=score.player.id)

            for server_achievement in server_achievements:
                if any(pa["achid"] == server_achievement["id"] for pa in player_achievements):
                    continue
                if server_achievement["cond"](score, score.mode.as_vanilla):
                    await user_achievements_usecases.create(score.player.id, server_achievement["id"])
    ...
    log(
        f"[{score.mode!r}] {score.player} submeteu score! "
...
async def get_leaderboard_scores(
    ...
    is_v2_request = (mods_int & SCORE_V2_BIT) != 0
    
    if is_v2_request:
        mod_filter = "AND (s.mods & :v2_mod) != 0"
    else:
        mod_filter = "AND (s.mods & :v2_mod) = 0"
    ...
    query = [
        ...
        "s.nmiss, s.nkatu, s.ngeki, s.perfect, s.mods, s.status, "
        "UNIX_TIMESTAMP(s.play_time) AS time, u.id AS userid, "
    ...
    personal_best_score_row = await app.state.services.database.fetch_one(
        ...
        "nmiss, nkatu, ngeki, perfect, mods, status, "
        "UNIX_TIMESTAMP(play_time) AS time "
        "FROM scores s "
        "WHERE s.map_md5 = :map_md5 AND s.mode = :mode "
...
async def getScores(
    ...
    """Endpoint oficial que o cliente do osu! chama para carregar o ranking."""
    if not player:
        return Response(b"error: pass")

    if mods_arg & Mods.RELAX:
        if mode_arg == 3: mods_arg &= ~Mods.RELAX
        else: mode_arg += 4
    elif mods_arg & Mods.AUTOPILOT:
        if mode_arg in (1, 2, 3): mods_arg &= ~Mods.AUTOPILOT
        else: mode_arg += 8
    ...
    return Response(content.encode("utf-8"), media_type="text/plain")
