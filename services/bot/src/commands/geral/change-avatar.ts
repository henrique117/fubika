import { executeChangeAvatar } from "../../services/logic/changeAvatar.logic"
...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        try {
            const result = await executeChangeAvatar(interaction.user.id, attachment.url)

            if (!result.success) {
                const embed = await defaultEmbedBuilder(result.error || 'Erro ao atualizar avatar')
                await interaction.editReply({ embeds: [embed] })
                return
            }
